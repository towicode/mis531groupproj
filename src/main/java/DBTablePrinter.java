...
public class DBTablePrinter {
	 ...
	 */
	public static ArrayList<String> printResultSet(ResultSet rs) {
	 ...
	 * @param maxStringColWidth Max. width of text columns
	 */
	public static ArrayList<String> printResultSet(ResultSet rs, int maxStringColWidth) {
		...
		try {
			...
			for (int i = 0; i < rowCount; i++) {
				for (Column c : columns) {
					...
					r.add(String.format(format, c.getValue(i))
					      );
				}
